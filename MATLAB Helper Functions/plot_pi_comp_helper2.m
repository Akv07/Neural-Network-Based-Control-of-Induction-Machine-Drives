...
legend('Reference', 'Ts = 1e-4', 'Ts = 5e-4');
...
plot(tim, ref_d2, ':', 'color', [0 0.5 0], 'LineWidth', 2);
hold on;
tim = linspace(0,4,size(ref_d1,1));
plot(tim, ref_d1, '--', 'color', 'blue', 'LineWidth', 2);
hold on;
plot(tim, meas_d1, 'color', 'red','LineWidth', 2);
xlabel('Time (in seconds)');
ylabel('Measured d Axis Current and Reference (in A)');
legend('Ts = 5e-4', 'Ts = 1e-4', 'Reference');
